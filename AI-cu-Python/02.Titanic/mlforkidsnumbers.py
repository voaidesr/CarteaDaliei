...
class MLforKidsNumbers:
        ...
        try:
            with urlopen(status_url) as url:
                ...
                if project_info["status"] != "Available":
                    ...
                    raise Exception(
                        "The model is not available for use - the current status is "
                        + project_info["status"]
                    )
            ...
            if e.code == 404:
                ...
                self._message(
                    "Train a new model on the Machine Learning for Kids site, then try again with the new URL."
                )
    ...
    def _download_model(self, status_url, model_folder):
        ...
        self._download_file(
            urljoin(model_info["urls"]["model"], "model.zip"), model_zip
        )
        self._download_file(status_url, join(model_folder, "mlforkids.json"))
    ...
    def store(self, data, label):
        if self._scratchkey is None:
            self._message(
                "You need to provide a key to be able to add to your training data"
            )
            self._message(
                "This can only be done for projects that are stored in the cloud"
            )
